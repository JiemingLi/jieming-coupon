...
public class RedisServiceImpl implements IRedisService {
    ...
    @Override
    public void addCouponFromExpiredToUsable(Long userId, List<Coupon> coupons) throws CouponException {
        log.debug("Add Coupon From Expired To Cache For Usable.");
        // 最终需要保存的 Cache
        ...
        coupons.forEach(c -> needCachedForUsable.put(
                c.getId().toString(),
                JSON.toJSONString(c)
        ));
        ...
        System.err.println("clean keys:" + needCleanKey);
        redisTemplate.opsForHash().putAll(redisKeyForUsable,needCachedForUsable);
        redisTemplate.opsForHash().delete(redisKeyForExpired,JSON.toJSONString(needCleanKey.toArray()));
...
//                        redisKeyForUsable, needCachedForUsable
//                );
//                // 2. 可用的优惠券 Cache 需要清理
//                redisOperations.opsForHash().delete(
//                        redisKeyForExpired, needCleanKey.toArray()
//                );
//
//                // 3. 重置过期时间
...
//                ));
    ...
    @Override
    public void updateCacheTemplate(Long userId, List<Coupon> coupons, Integer status) throws CouponException {
...
//        Long n = redisTemplate.opsForHash().delete(redisKey,JSON.toJSONString(ids));
//        log.info("删除 " + n + "条记录");
//        redisTemplate.opsForHash().putAll(redisKey,needCachedObject);
